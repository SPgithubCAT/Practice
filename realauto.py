...
def get_target_price(ticker, k):
    ...
    df = pyupbit.get_ohlcv(ticker, interval=set_interval, count=set_count)
...
while True:
    try:
        ...
        end_time = start_time + datetime.timedelta(minutes=60) # 캔들 조정!!!
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=5):
            ...
            if target_price < current_price:
                if df2.iloc[c1,7] > df2.iloc[c1,8]:
                    krw = get_balance("KRW")
                    if krw > 6000:
